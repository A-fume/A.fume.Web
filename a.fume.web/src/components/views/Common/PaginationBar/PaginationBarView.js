        ...
        <div className="pagination_bar_container">
            {1 < props.pagingIndex && (
                <div
                    ...
                    onClick={() =>
                        props.onUpdatePagingIndex(
                            parseInt(props.pagingIndex) - 1
                        )
                    }
                >
                    <DoubleLeftOutlined />
            ...
            {Pages &&
                Pages.map((it, index) => {
                    const pageView =
                        parseInt(props.pagingIndex) === it ? (
                            <span className="pagination_btn_page active">
                                {it}
                            </span>
                        ) : (
                            <div
                                className="pagination_btn_page"
                                onClick={() => props.onUpdatePagingIndex(it)}
                            >
                                {it}
                            </div>
                        );
                    return (
                        <React.Fragment key={index}>{pageView}</React.Fragment>
                    );
                })}
            {props.pagingIndex < props.totalPage && (
                <div
