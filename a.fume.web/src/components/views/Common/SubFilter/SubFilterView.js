...
function SubFilterView(props) {
    ...
    const [AscendingIdx, setAscendingIdx] = useState((defaultAscending === 'asc' ? 2 : (defaultAscending === 'desc' ? 1 : 0)));
    ...
    return (
        <div className='sub_filter_container'>
            {
                sortCriteriaList && sortCriteriaList.map((sortCriteria, index) => (
                    <React.Fragment key={index}>
                        <span className='sort_criteria' 
                        ...
                        {Key === sortCriteria.value && 
                            (
                                (AscendingIdx === 1 && <CaretDownOutlined />)
                                || (AscendingIdx === 2 && <CaretUpOutlined />)
                            )
                        }
                        </span>
                    </React.Fragment>
            ...
            <Select defaultValue={props.pagingSize} style={{ width: 120 }} onChange={(it) => {props.onUpdate({pagingSize: it})}}>
